...
@section('content')
                            ...
                            <tbody>
                                @foreach($data as $d)
                                    <tr>
                                        ...
                                        <td>{{ $d['created_at'] }}</td>
                                        <td>
                                            <button onclick="update(`{{ $d['id'] }}|{{ $d['judul'] }}|{{ $d['penulis'] }}|{{ $d['foto'] }}|{{ $d['deskripsi'] }}|{{ $d['url'] }}`)" class="btn btn-primary" data-toggle="modal" data-target="#modal"><i class="menu-icon ti-pencil"></i> Ubah</button>
    ...
    <script type="text/javascript">
        ...
        function update(val)
        {
            ...
            $('#judul').val(val[1])
            $('#sub_judul').val(val[2])
            $('#image-preview').attr('src', `{{ base_url }}testimonials/${val[3]}`)
            $('#deskripsi').val(val[4])
            $('#url').val(val[5])
        }
        ...
        $('#form').submit(ev =>
        {
            ...
            $.ajax({
                url: `{{ base_url }}admin/testimonial/${url}`,
                method: method,
                data: data,
                processData: false,
                contentType: false,
